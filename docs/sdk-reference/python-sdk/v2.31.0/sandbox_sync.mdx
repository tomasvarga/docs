...
```python
def send_stdin(data: Union[str, bytes],
...
```python
def close_stdin(request_timeout: Optional[float] = None) -> None
```
...
```python
def close_stdin(pid: int, request_timeout: Optional[float] = None) -> None
```
...
```python
def stop(request_timeout: Optional[float] = None)
```
...
```python
def get_new_events(
        request_timeout: Optional[float] = None) -> List[FilesystemEvent]
```
...
def read(path: str,
         ...
         request_timeout: Optional[float] = None,
         gzip: bool = False,
         stream_idle_timeout: Optional[float] = None) -> FileStreamReader
```
...
File content as a `FileStreamReader`
...
```python
def next_items(**opts: Unpack[ApiParams]) -> List[SandboxInfo]
```
...
```python
def next_items(**opts: Unpack[ApiParams]) -> List[SnapshotInfo]
```
...
- `volume_mounts`: Dictionary mapping mount paths to Volume instances or volume names
...
```python
@overload
def update_network(network: SandboxNetworkUpdate,
                   **opts: Unpack[ApiParams]) -> None
```
...
@overload
@staticmethod
def update_network(sandbox_id: str, network: SandboxNetworkUpdate,
                   **opts: Unpack[ApiParams]) -> None
```
...
```python
@overload
def pause(keep_memory: bool = True, **opts: Unpack[ApiParams]) -> bool
```
...
```python
@class_method_variant("_cls_pause")
def beta_pause(keep_memory: bool = True, **opts: Unpack[ApiParams]) -> bool
```
...
A `SandboxPaginator` that yields pages of sandboxes (running and paused by default). Iterate pages via `paginator.next_items()` while `paginator.has_next` is True.
