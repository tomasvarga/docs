...
```python
async def send_stdin(data: str) -> None
```
...
async def write(path: str,
                ...
                request_timeout: Optional[float] = None,
                gzip: bool = False,
                use_octet_stream: bool = False) -> WriteInfo
```
...
```python
async def write_files(files: List[WriteEntry],
                      user: Optional[Username] = None,
                      request_timeout: Optional[float] = None,
                      gzip: bool = False,
                      use_octet_stream: bool = False) -> List[WriteInfo]
```
