...
```ts
read(path: string, opts?: FilesystemRequestOpts & object): Promise<Uint8Array<ArrayBufferLike>>
```
...
```ts
read(path: string, opts?: FilesystemRequestOpts & object): Promise<ReadableStream<Uint8Array<ArrayBufferLike>>>
```
