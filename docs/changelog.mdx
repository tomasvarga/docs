...
<Update label="July 14, 2026" description="Template build reliability fixes" tags={["Templates", "SDK"]}>

## Bug fixes

- Fixed an issue where template builds could silently drop files during a `COPY` step when the destination directory already existed, so `COPY` now merges into existing directories the way Docker does (see [Template quickstart](/docs/template/quickstart)).
- Fixed template builds occasionally producing sandboxes with less free disk than expected after an unclean host restart, so the free space calculation is now accurate.
- Fixed template builds reporting a failed step as successful under a rare race, so failing build commands now consistently surface the underlying error.
