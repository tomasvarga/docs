...
[Grok Build](https://x.ai/cli) is xAI's coding agent and CLI. E2B provides a pre-built `e2b/grok` template with Grok Build already installed.
...
const sandbox = await Sandbox.create('e2b/grok', {
...
sandbox = Sandbox.create("e2b/grok", envs={
...
If you need to customize the environment (e.g. pre-install dependencies, add config files), build your own template on top of the pre-built `e2b/grok` template.
